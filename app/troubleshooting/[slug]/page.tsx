...
export async function generateStaticParams() {
  return getAllSlugs("troubleshooting").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const article = getArticle("troubleshooting", slug);
  if (!article) return {};
  return {
...
export default async function IssueDetailPage({ params }: Props) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <Breadcrumb
        items={[{ label: "Troubleshooting", href: "/troubleshooting" }, { label: article.frontmatter.title }]}
      />
      ...
      <PrevNextNav
        prev={{ label: "Back to Troubleshooting", href: "/troubleshooting" }}
        next={null}
      />

      <DiscussionSection slug={slug} docType="troubleshooting" />
    </main>
  );
}
